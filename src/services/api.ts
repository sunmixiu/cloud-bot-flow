...
export const API_CONFIG = {
  // 配置了 VITE_API_BASE_URL 即视为接入外部/真实 cube-studio，关闭本地演示分支
  ...
  // cube-studio AUTH_PLATFORM_ACCESS：Authorization 直接传用户名
  authToken: import.meta.env.VITE_CUBE_AUTH_TOKEN || 'admin',
...
// 获取认证头（真实 cube-studio：短 Authorization = 用户名）
export const getAuthHeaders = () => {
  ...
  const raw = (token || username || API_CONFIG.authToken || 'admin').replace(/^Bearer\s+/i, '');
  return {
...
// 资源相关API（真实 cube-studio 无 robot_modelview 时回退本地 JSON）
export const resourceApi = {
  // 获取机器人数据
  getRobots: async () => {
    try {
      const response = await apiRequest<any>(API_ENDPOINTS.robots);
      return response?.result?.data || response;
    } catch {
      const res = await fetch('/data/robots.json');
      if (!res.ok) throw new Error('加载机器人数据失败');
      return res.json();
    }
  },
